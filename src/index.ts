  ...
  for (const service of services) {
    ...
    for (const operation of operations) {
      for (const message of operation.messages()) {
        ...
        const eventOwnership = message.extensions().get('x-eventcatalog-message-ownership')?.value() || 'internal';
        const isOwnedByTheService = eventOwnership === 'internal';
